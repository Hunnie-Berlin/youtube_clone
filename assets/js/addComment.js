...
const increseViewNumber = () => {
  const currnetNumber =
    commentNumber.innerText.split(" ")[0] === "No"
      ? "0"
      : commentNumber.innerText.split(" ")[0];
...
const addComment = (comment, commentId) => {
  ...
  div.setAttribute("class", "comment-avatar");
  div.setAttribute("src", `${avatar.innerHTML}`);
  const li = document.createElement("li");
  li.setAttribute("class", commentId);
  ...
  if (avatar.innerHTML !== "") {
    ...
    div.innerText = name.innerText[0];
    li.appendChild(div);
  }
...
const sendComment = async (comment) => {
  ...
  if (response.status === 200) {
    const commentId = response.headers.commentid;
